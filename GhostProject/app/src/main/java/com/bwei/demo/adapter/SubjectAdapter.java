...
public class SubjectAdapter extends RecyclerView.Adapter<SubjectAdapter.Myviewholder> {
    ...
    private List<RecommendBean.RetBean.ListBean.ChildListBean> videoInfo;
    private Context context;

    public SubjectAdapter(List<RecommendBean.RetBean.ListBean.ChildListBean> videoInfo, Context context) {
    ...
    public Myviewholder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        final View inflate = LayoutInflater.from(context).inflate(R.layout.item, parent, false);
    ...
    public void onBindViewHolder(Myviewholder holder, int position) {
        ...
        if(videoInfo.get(position).getPic().length()>0&&videoInfo.get(position).getTitle().length()>0){
            holder.textView.setText(videoInfo.get(position).getTitle());
...
//          int height = data.getHeight()*width/data.getWidth();//计算View的高度
            ...
            Glide.with(context).load(videoInfo.get(position).getPic()).placeholder(R.mipmap.ic_launcher).into(holder.imageViewl);
